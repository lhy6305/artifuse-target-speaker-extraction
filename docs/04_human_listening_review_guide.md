...
如果遇到下面这种情况，优先考虑记 `prefer_silence_over_leak`：

- 目标本身已经很弱、接近不可辨
- 一边不是保留了更多可用源，而是主要放大了干扰残留
- 另一边虽然几乎静音，但作为后级 VC / TTS / editing 输入更干净

这条规则的核心不是“越安静越好”，而是：

- 目标仍可用时，保留更多可用目标通常更好
- 目标已不可用时，`闭嘴` 常常比 `输出干扰` 更好
...
- 一边几乎静音，但另一边主要只剩干扰；这种情况要明确写是否“宁可闭嘴”
